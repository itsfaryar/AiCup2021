...
public class main {
	public  enum Status{SEARCHING,GO_FOR_BOX,RUN_FROM_BOMB,Go_FOR_ITEM,PLACE_BOMB,GOTO_CENTER,KNIFE_PLAYER}
	...
	public class Game {
		...
		public void findPosForRunFromBomb() {
			
			Position t=new Position(pl.x,pl.y);
			boolean flag=true;
			if(pl.x-1>0 && pl.y-1>0 && map[pl.x-1][pl.y-1]==STATE_ZONE) {
				if(flag) {
					t.x=pl.x-1;
					t.y=pl.y-1;
					flag=false;
			...
			if(pl.x+1<h && pl.y+1<w && map[pl.x+1][pl.y+1]==STATE_ZONE) {
				if(flag) {
					t.x=pl.x+1;
					t.y=pl.y+1;
					flag=false;
				}
				else if(getdist(pl.x+1,pl.y+1)<getdist(t,pl.x,pl.y)) {
					t.x=pl.x+1;
					
				}
				
			}
			if(pl.y-1>0 && pl.x+1<h &&map[pl.x+1][pl.y-1]==STATE_ZONE) {
				if(flag) {
					t.x=pl.x+1;
					t.y=pl.y-1;
					flag=false;
			...
			if(pl.y+1<w && pl.x-1>0 && map[t.x-1][pl.y+1]==STATE_ZONE) {
				if(flag) {
					t.y=pl.y+1;
					t.x=pl.x-1;
					flag=false;
			...
			safe_place=t;
			System.err.println("safe place: "+safe_place);
		}
		public void updateTile(int x,int y, int state) {
			...
			else if(state==STATE_BOMB || state==STATE_PLAYERWITHBOMB){
				is_bomb=true;
		...
		public boolean is_player(int inp) {
			if(inp==STATE_PLAYER || inp==STATE_PLAYER_IN_DEADZONE ||inp==STATE_PLAYERWITHBOMB)return true;
			return false;
		}
		public int knifePlayer() {
			if(pl.x-1>0 && is_player(map[pl.x-1][pl.y]))return ACTION_TRAP_UP;
			if(pl.x+1<h && is_player(map[pl.x+1][pl.y]))return ACTION_TRAP_DOWN;
			if(pl.y-1>0 && is_player(map[pl.x][pl.y-1]))return ACTION_TRAP_LEFT;
			if(pl.y+1<w && is_player(map[pl.x][pl.y+1]))return ACTION_TRAP_RIGHT;
			...
			System.err.println(" deadzoneStartingStep: "+deadzoneStartingStep);
				...
				if(state==Status.SEARCHING) {
					...
					if(is_bomb) {
						...
						bomb_ticker=0;
						findPosForRunFromBomb();
						System.err.println("safe place: "+safe_place);
					}
					else if(k!=-1 && pl.trapCount>0) {
						state=Status.KNIFE_PLAYER;
					}
					else if(deadzone) {
						findNearestFromCenter();
						state=Status.GOTO_CENTER;
					}
				...
				if(state==Status.GO_FOR_BOX) {
				...
				else if(state==Status.KNIFE_PLAYER) {
					if(k!=-1) {
						action=k;
					}
					else {
				...
				else if(state==Status.GOTO_CENTER) {
					int t= printShortestDistance(convertTo1D(pl.x, pl.y),convertTo1D(nearToCenter.x,nearToCenter.y));
					Position p=convertTo2D(t);
					action=moveToTile(p.x, p.y);
					state=Status.SEARCHING;
					...
					if(pl.x==safe_place.x && pl.y==safe_place.y) {
						action=ACTION_STAY;
					}
					else {
						int t= printShortestDistance(convertTo1D(pl.x, pl.y),convertTo1D(safe_place.x,safe_place.y));
						Position p=convertTo2D(t);
						action=moveToTile(p.x, p.y);
					}
					...
					bomb_ticker++;
					System.err.println("bomb ticker"+bomb_ticker);
				}
